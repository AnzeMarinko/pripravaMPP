...
def pripraviHTML(datum):
    # uvoz strani z Bozjo besedo
    url = 'https://hozana.si/index.php?datum={}'.format(datum)
    HozanaHTML = requests.get(url).content.decode('utf-8').split("\n")
    ...
    # zapisi v datoteko
    file = open("izdelanePriprave\{}-{}-{}.html".format(datum[8:10],datum[3:5],datum[0:2]),"w") 
    file.write(html)
    file.close() 
    return html
